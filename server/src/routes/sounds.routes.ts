...
soundsRouter.get("/", async (req: Params, res) => {
  const files = await sendAction(req.params.deviceId, "sounds.listAll");
  res.json(files);
});

soundsRouter.post("/", async (req: Params, res) => {
  const result = await sendAction(req.params.deviceId, "sounds.upload", req.body, uploadTimeoutMs);
  res.json(result);
});

soundsRouter.delete("/:name", async (req: DeleteParams, res) => {
  await sendAction(req.params.deviceId, "sounds.delete", { name: req.params.name });
  res.status(204).end();
});
