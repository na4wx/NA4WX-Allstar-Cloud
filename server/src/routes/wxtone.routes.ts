...
// Reads stay at authorizeDevice's own default (viewer and up); writes
// require editor.
...
wxtoneRouter.post("/", requireDeviceRole("editor"), async (req: ListParams, res) => {
  const saved = await auditedSendAction(req, "wxTone.save", req.body);
  res.json(saved);
});

wxtoneRouter.delete("/:id", requireDeviceRole("editor"), async (req: DeleteParams, res) => {
